...
from django.db import transaction  # <-- EKLENDİ
...
class UrunViewSet(viewsets.ModelViewSet):
    ...
    @action(detail=False, methods=['get'], url_path='filtrele',permission_classes=[AnyRole("yonetici","depo_gorevlisi","personel")])
    ...
    # (ÇAKIŞMAYI ÖNLEMEK İÇİN SADECE BUNU BIRAKTIK)
    @action(
        detail=False,
        methods=['get'],
        url_path='qr-bul',
    ...
    # /urunler/modeller/?ad=...
    @action(detail=False, methods=['get'], url_path='modeller',permission_classes=[AnyRole("yonetici", "depo_gorevlisi","personel")])
    ...
    @action(detail=False, methods=['get'], url_path='filtre-secimleri',permission_classes=[AnyRole("yonetici", "depo_gorevlisi","personel")])
    ...
    # /urunler/adlar/
    @action(detail=False, methods=['get'], url_path='adlar',permission_classes=[AnyRole("yonetici", "depo_gorevlisi","personel")])
    def adlari_getir(self, request):
        adlar = Urun.objects.values_list('ad', flat=True).distinct()
        return Response({"adlar": list(adlar)}, status=status.HTTP_200_OK)

    # /urunler/{pk}/detayli-bilgi/
    @action(detail=True, methods=["get"], url_path="detayli-bilgi",permission_classes=[AnyRole("yonetici","personel")])
    def detayli_bilgi(self, request, pk=None):
        urun = self.get_object()  # DRF bulunamazsa 404 fırlatır
    ...
    def urun_durum(self, request):
        ...
        # Daha okunaklı dictionary formatına çevir
        sonuc = {item["durum"]: item["sayi"] for item in durum_sayilari}
    ...
    @action(detail=True, methods=['post'], url_path='durum-degistir',permission_classes=[AnyRole("yonetici", "depo_gorevlisi","personel")])
    ...
    def detayli_liste(self, request):
        ...
        # Geçmişi tek sorguda çek ve tarihe göre sırala
