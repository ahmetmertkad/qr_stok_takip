...
class UrunDetayliListeSayfasi extends StatefulWidget {
  final String token;
  const UrunDetayliListeSayfasi({super.key, required this.token});
...
class _UrunDetayliListeSayfasiState extends State<UrunDetayliListeSayfasi> {
  ...
  void initState() {
    super.initState();
    // setState hiç çağırmadan, ilk frame’den sonra veri çekmeyi başlat
    WidgetsBinding.instance.addPostFrameCallback((_) {
      _yukle(); // bu fonksiyon içinde setState SYNCHRONOUS
    });
  }

  void _yukle() {
    // 1) future’ı hazırla (asenkron iş setState DIŞINDA başlar)
    final f = UrunService.detayliListe(
      durumList: _seciliDurumlar.isEmpty ? null : _seciliDurumlar.toList(),
      limit: _limit,
    );

    // 2) tek seferde state güncelle (SYNC)
    setState(() {
      _loading = true;
      _future = f;
    });

    // 3) future bitince loading’i kapat (SYNC)
    f.whenComplete(() {
      if (!mounted) return;
      setState(() {
        _loading = false;
  ...
  Future<void> _yenile() async {
    _yukle();
    final f = _future;
    if (f != null) await f; // bekleme setState DIŞINDA
  }
      ...
      appBar: AppBar(
        ...
        actions: [
          IconButton(
            ...
            onPressed: chipsDisabled ? null : () => _durumFiltreSheet(context),
          ),
        ],
          ...
          child: Wrap(
            ...
            children:
                durumlar.map((d) {
                  ...
                  return FilterChip(
                    selected: secili,
                    onSelected: chipsDisabled ? null : (_) => _toggleDurum(d),
                    label: Text(d),
  ...
  void _durumFiltreSheet(BuildContext context) {
        ...
        return SafeArea(
          child: Padding(
            ...
            child: StatefulBuilder(
              builder:
                  (context, setSt) => SingleChildScrollView(
                    child: Column(
                      mainAxisSize: MainAxisSize.min,
